...
def render_comparison(analysis):
    ...
    def cell(clause,parts,side):
        ...
        source='<div class="source">'+esc(display_name(clause.get("document","")))+'</div>' if len(data["documents"][side])>1 else ''
        return source+'<div class="ref">'+esc(citation(clause))+'</div><div class="text">'+text+'</div>'
    before="; ".join(display_name(d["name"]) for d in data["documents"]["before"])
    after="; ".join(display_name(d["name"]) for d in data["documents"]["after"])
    rows=[]
    ...
    return '<!doctype html><html lang="ru"><meta charset="utf-8"><title>Versa — сравнительная таблица</title><style>'+style+'</style><body><h1>Versa · Сравнительная таблица</h1><p>Положения сопоставлены последовательно, с указанием их обозначений в каждом документе. Красным зачёркнутым шрифтом выделены исключённые формулировки; красным жирным шрифтом — включённые формулировки. Пояснения отражают различия между документами; вывод о соответствии законодательству требует отдельной правовой оценки.</p><table><thead><tr><th>Документ 1<br>'+esc(before)+'</th><th>Документ 2<br>'+esc(after)+'</th><th>Содержание изменений и дополнений</th></tr></thead><tbody>'+"".join(rows)+'</tbody></table></body></html>'
